...
- Never arm live until `LOOP.md` constraints 1–2 exist in code (preview-token handshake + IBKR account-identity check)
...
5. Only then consider live trading (with user approval at every step) — and only after the handshake + identity checks in `LOOP.md` ship
